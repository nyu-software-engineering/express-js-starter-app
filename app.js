...
app.get('/middleware-example', (req, res) => {
    ...
    const message = (res.addedStuff) ? res.addedStuff : 'Sorry, the middleware did not work!'
...
app.get('/proxy-example', (req, res, next) => {
    ...
    axios.get("https://my.api.mockaroo.com/animals.json?key=d9ddfc40&num=10")
...
app.get('/dotenv-example', (req, res, next) => {
    ...
    axios.get(`${process.env.API_BASE_URL}?key=${process.env.API_SECRET_KEY}&num=10`)
    .then(apiResponse => res.json(apiResponse.data)) // pass data along directly to client
    .catch(err => next(err)); // pass any errors to express
});
